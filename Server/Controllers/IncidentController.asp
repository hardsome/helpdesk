      ...
      */
      var IncidentController = (function() {
         ...
         var Create = function(callback){
            ...
            obj.IncidentName = validator.toString(args["IncidentName"]);
            obj.ProjectID = validator.toInt(args["ProjectID"]);
            obj.StatusID = validator.toInt(args["StatusID"]);
            obj.PriorityID = validator.toInt(args["PriorityID"]);
            obj.CategoryID = validator.toInt(args["CategoryID"]);
            obj.Description = validator.toString(args["Description"]);
            obj.ReportedDate = validator.toDate(args["ReportedDate"]);
            obj.UpdatedDate = validator.toDate(args["UpdatedDate"]);
            obj.UserID = validator.toInt(args["UserID"]);
            
            var result = IncidentHelper.Insert(obj);
         ...
         var Read = function (callback,vars){
            ...
            if (vars){
                ...
                if (validator.isInt(id)&&(id>0)) {
                   model = {data: IncidentHelper.SelectById(String(id))};
                }
                else
                   model = {data : new Incident()};
            }
            callback(JSON.stringify(model),"application/json") ;
         };
         ...
         var Update = function (callback){
            ...
            if (isInt(Number(id))){
                var obj;
                obj = IncidentHelper.SelectById(id);
                obj.IncidentName = validator.toString(args["IncidentName"]);
                obj.ProjectID = validator.toInt(args["ProjectID"]);
                obj.StatusID = validator.toInt(args["StatusID"]);
                obj.PriorityID = validator.toInt(args["PriorityID"]);
                ...
                var result =  IncidentHelper.Update(obj);
         ...
         var Delete = function(callback){
            ...
            if (validator.isInt(Number(id))){
                model = {result :  IncidentHelper.Delete(id) ,data : {id : id} };
            }
            callback(JSON.stringify(model),"application/json") ;
         };
         ...
         var PagedList = function(callback,vars){
              ...
              var model = {result: false};
              if (searchstring!=''){
                model = {data : {items : IncidentHelper.Search(searchstring) }};
                callback(JSON.stringify(model),"application/json") ;
                return;
              }
              ...
              var paginator = {
                ...
                first     : 1,
                perpage   : 15,
                total     :IncidentHelper.Count()
                };
              if 	(paginator.total<paginator.perpage*3){
                paginator = null; //not needed
                model = {data : {items:IncidentHelper.SelectAll()} };
              }
              else{
                ...
                model = {
	                data : { 
                    items: IncidentHelper.SelectPage(paginator.current,paginator.perpage,'id'),
	                  paginator : paginator
                  }
                };
              }
         ...
         var BasicList = function(callback){
            var model = {data : {items: IncidentHelper.SelectAll()} };
            callback(JSON.stringify(model),"application/json") ;
         };
        
         //read operation, only id and name
         var DropDown = function(callback){
            var model = {data : IncidentHelper.DropDown() };
            callback(JSON.stringify(model),"application/json") ;
         };
