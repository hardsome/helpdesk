      ...
      */
      var AttachmentController = (function() {
         ...
         var Create = function(callback){
            ...
            obj.AttachmentName = validator.toString(args["AttachmentName"]);
            obj.Incident = validator.toInt(args["Incident"]);
            obj.Creator = validator.toInt(args["Creator"]);
            obj.Content = validator.toString(args["Content"]);
            obj.MimeType = validator.toString(args["MimeType"]);
            
            var result = AttachmentHelper.Insert(obj);
         ...
         var Read = function (callback,vars){
            ...
            if (vars){
                ...
                if (validator.isInt(id)&&(id>0)) {
                   model = {data: AttachmentHelper.SelectById(String(id))};
                }
                else
                   model = {data : new Attachment()};
            }
            callback(JSON.stringify(model),"application/json") ;
         };
         ...
         var Update = function (callback){
            ...
            if (isInt(Number(id))){
                var obj;
                obj = AttachmentHelper.SelectById(id);
                obj.AttachmentName = validator.toString(args["AttachmentName"]);
                obj.Incident = validator.toInt(args["Incident"]);
                obj.Creator = validator.toInt(args["Creator"]);
                obj.Content = validator.toString(args["Content"]);
                ...
                var result =  AttachmentHelper.Update(obj);
         ...
         var Delete = function(callback){
            ...
            if (validator.isInt(Number(id))){
                model = {result :  AttachmentHelper.Delete(id) ,data : {id : id} };
            }
            callback(JSON.stringify(model),"application/json") ;
         };
         ...
         var PagedList = function(callback,vars){
              ...
              var model = {result: false};
              if (searchstring!=''){
                model = {data : {items : AttachmentHelper.Search(searchstring) }};
                callback(JSON.stringify(model),"application/json") ;
                return;
              }
              ...
              var paginator = {
                ...
                first     : 1,
                perpage   : 15,
                total     :AttachmentHelper.Count()
                };
              if 	(paginator.total<paginator.perpage*3){
                paginator = null; //not needed
                model = {data : {items:AttachmentHelper.SelectAll()} };
              }
              else{
                ...
                model = {
	                data : { 
                    items: AttachmentHelper.SelectPage(paginator.current,paginator.perpage,'id'),
	                  paginator : paginator
                  }
                };
              }
         ...
         var BasicList = function(callback){
            var model = {data : {items: AttachmentHelper.SelectAll()} };
            callback(JSON.stringify(model),"application/json") ;
         };
        
         //read operation, only id and name
         var DropDown = function(callback){
            var model = {data : AttachmentHelper.DropDown() };
            callback(JSON.stringify(model),"application/json") ;
         };
