      ...
      */
      var CategoryController = (function() {
         ...
         var Create = function(callback){
            ...
            obj.CategoryName = validator.toString(args["CategoryName"]);
            obj.SortOrder = validator.toInt(args["SortOrder"]);
            
            var result = CategoryHelper.Insert(obj);
         ...
         var Read = function (callback,vars){
            ...
            if (vars){
                ...
                if (validator.isInt(id)&&(id>0)) {
                   model = {data: CategoryHelper.SelectById(String(id))};
                }
                else
                   model = {data : new Category()};
            }
            callback(JSON.stringify(model),"application/json") ;
         };
         ...
         var Update = function (callback){
            ...
            if (isInt(Number(id))){
                var obj;
                obj = CategoryHelper.SelectById(id);
                obj.CategoryName = validator.toString(args["CategoryName"]);
                obj.SortOrder = validator.toInt(args["SortOrder"]);
                
                var result =  CategoryHelper.Update(obj);
         ...
         var Delete = function(callback){
            ...
            if (validator.isInt(Number(id))){
                model = {result :  CategoryHelper.Delete(id) ,data : {id : id} };
            }
            callback(JSON.stringify(model),"application/json") ;
         };
         ...
         var PagedList = function(callback,vars){
              ...
              var model = {result: false};
              if (searchstring!=''){
                model = {data : {items : CategoryHelper.Search(searchstring) }};
                callback(JSON.stringify(model),"application/json") ;
                return;
              }
              ...
              var paginator = {
                ...
                first     : 1,
                perpage   : 15,
                total     :CategoryHelper.Count()
                };
              if 	(paginator.total<paginator.perpage*3){
                paginator = null; //not needed
                model = {data : {items:CategoryHelper.SelectAll()} };
              }
              else{
                ...
                model = {
	                data : { 
                    items: CategoryHelper.SelectPage(paginator.current,paginator.perpage,'id'),
	                  paginator : paginator
                  }
                };
              }
         ...
         var BasicList = function(callback){
            var model = {data : {items: CategoryHelper.SelectAll()} };
            callback(JSON.stringify(model),"application/json") ;
         };
        
         //read operation, only id and name
         var DropDown = function(callback){
            var model = {data : CategoryHelper.DropDown() };
            callback(JSON.stringify(model),"application/json") ;
         };
